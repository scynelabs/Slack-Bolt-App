...
const queryCaseDetail = async (connection) => {
    try {
        ...
        const result = await connection.query(
            ...
            `SELECT Id, Work_Type_Group__r.Name, CaseNumber, Status, Licence_Type__c, Subject, Priority, Application_Date__c, Type, Account.Name FROM Case WHERE id='500Kj00001aT9qdIAC'`
        );
        return result;
    } catch (e) {
        throw new Error(e.message);
